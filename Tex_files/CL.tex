...
\begin{figure*}
  ...
  \caption{Reconstructed $\kappa$ and deflection angle $\alpha$ profiles from  stacking a set of $1000$ temperature-only reconstructions ($M = 4 \cdot 10^{14} M_\odot, z=0.7$). CMB multipoles $100 \leq \ell \leq 5000$ are used to reconstruct the lensing multipoles $100 \leq L \leq 5000$. Shown are the standard QE (blue), QE with a high-$\ell$ cut at $2000$ on the gradient leg~\cite{Hu:2007bt}, and the maximum a posteriori (MAP) lensing reconstruction of this work (red, without any cuts), and the input truncated to the same $L$ range. The bias in the MAP reconstruction is much reduced compared to the QE but still visible at this cluster mass. In polarization reconstruction no such bias is visible.\JC{QE and MAP, right ?}}
  \label{fig:Bias_sup}
  \end{figure*}



As discussed in the literature \cite{Hu:2007bt, Maturi:2004zj, Amblard:2004ih} \LL{are you sure the last ref is about the same bias?}\JC{That ref does not seem right indeed}, the temperature quadratic estimator is biased low due to strong to moderate lensing close to the cluster center. 
...
\subsection{CMB fluctuation noise suppression}
