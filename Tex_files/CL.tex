...
This gives the normalised QE convergence field in Fourier space\JC{This equation looks wrong. You mean $i\vL\cdot \hat \valpha^{\rm QE}(\vL)$?}
\begin{equation}
...
\begin{figure}
	...
	\caption{The similar plot as the lower pannel of Fig.~\ref{fig:Bias_sup}, where we plot the Eq.~\ref{Eq:Int_kappa0} for QU channel estimators. As expected the QU channel QE does not suffer from the bias due to strong to moderate lensing close to the cluster center.}
	%The MAP result appears slightly less biased than HDV QE but this is coincidental, see Fig.~\ref{fig:Bias_sup}
...
One difference is that the analytical predictions are calculated using curved-sky geometry, while the simulations use small boxes in the flat-sky approximation. Fig.~\ref{fig:biaspred} shows the prediction with this tool for the halo mass bias as a function of halo mass, together with our findings on this simulation set. In Fig.~\ref{fig:bias_sup_QU}, we plot the quantity in Eq.~\ref{Eq:Int_kappa0}, which is used to estimate $\kappa_0$, QU channel estimators. Intuituvely, the bias in T channel QE, comes due to the apperent decrease of the background gradient due to the multiple lens effect close to the cluster centre. However, for QU channel estimator, we include all the combinations of the curl and divergence-free counter-part of the polarisation stokes parameters. Consequently, such strong-lens magnification witnessed by the pure T channel QE is not encountered in the QU channel QE. Hence we see in Fig.~\ref{fig:bias_sup_QU} that the QE does not suffer from such bias for the QU channel. \JC{Some explanation for this here}\JC{Fig 4 before fig3 ?}\bb{done}
...
Much work remains to be done. We did not consider contamination from foregrounds signal, such as the SZ effect, radio point sources or the Cosmic Infrared Background. The thermal SZ effect has a frequency dependence which allows to remove its contribution from the observed CMB maps, at the expense of increased variance. It is possible to reduce the loss of signal in the lensing reconstruction by tSZ cleaning only the gradient leg of the QE \cite{Madhavacheril:2018bxi, DES:2018myw, Patil_2020}. The kinematic SZ however cannot be distinguished from the lensing signal. A more meticulous examination in presence of such foreground contaminations in simulations~\cite{Raghunathan:2019tsz, Levy:2023moy} is essential to include in our analysis. Hence, in our future work, we plan to work on the  robustness of our MAP-based cluster mass estimator against these foregrounds. \JC{not clear what this sentence means at all}\bb{better?} . However, it is expected that the polarization foregrounds are small on the relevant scales, so our MAP estimator forecast in polarization should in principle be robust.
