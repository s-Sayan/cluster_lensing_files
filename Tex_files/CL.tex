...
\begin{table}
\centering
\caption{Summary of Results from T channel using all the estimators (QE, HDV QE and MAP) on 1000 Simulations with Input $\kappa_0 = 0.1514$. The cluster parameters are $M_{200}=4\times 10^{14},z=0.7$. CMB multipoles have been used from $\ell_{\text{min}}^{\text{CMB}}=100$ to $\ell_{\text{max}}^{\text{CMB}}=5000$. The table displays the mean $\kappa_0$ evaluated from simulations along with the corresponding errors. The QE exhibits a noticeable low bias beyond $3\sigma$, whereas the bias is significantly reduced for HDV QE and MAP estimators \bb{check}.}
...
Our iterative estimator in other hand, does manage to get rid of $65\%$ of the bias\JC{This sounds weird} \bb{will update after final plots on bias}, without wasting any information in the small scale regime.
...
We use the lensing multipoles beateen $L_{\text{min}}=100$ and $L_{\text{max}}=6000$ to obtain $\hat \kappa_0$ from Eq.~\ref{eq:kappa_0}. We employ the empirical noise, obtained from 1000 simulations, as noise of $\hat \kappa_{L}$ estimation $N_L$ in that equation. \bb{check}
