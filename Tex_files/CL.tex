...
% \JC{How much of a difference would it make, could we just quote quickly a number?}
...
% \LL{Comment on the paper of Zubeldia 2020, maybe not relevant here: The correlation of galaxy clusters with LSS also has an impact on the statistics of the cluster mass observable \cite{Zubeldia:2020knz}.}\JC{what does it say in a nutshell? (sorry, no time just now to go through it :-)}\LL{it is about characterizing the PDF of the observable (lensing mass) as a function of the true cluster mass, using simulations}\bb{needs JC's attention}\JC{What's now in black fine by me}
