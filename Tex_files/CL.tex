...
\title{Cluster profiles from iterative CMB lensing maps}
...
This gives the normalised QE convergence field in Fourier space\JC{normalization looks wrong}
\begin{equation}
...
\LL{At the small scales, the assumption of Eqs.~\ref{eq:kappa_0_noise} and \ref{eq:Noisespec} that the noise covariance $N(\vL, \vL')$ is diagonal breaks down: the non diagonal terms can be of the same order as the diagonal ones. This can be interpreted as a tight correlation between modes that share the same large scale gradient \cite{Horowitz:2017iql}. The diagonal assumption of Eq.~\ref{eq:kappa_0_noise} then leads to an underestimate of the variance of $\hat \kappa_0$. This is particularly true for the QE. For the MAP, Ref.~\cite{Legrand:2021qdu}}\JC{slightly confused here by this discussion, spectrum covariance or map covariance?} \LL{showed that the covariance matrix is well approximated by diagonal, so Eq.~\ref{eq:kappa_0_noise} is a better approximation.}\JC{That ref shows that the power spectrum covariance is close to diagonal, not the covariance of $\hat \kappa(L)$ ? (are we able to quickly look at that covariance ?)}
...
\begin{figure}
    ...
    \caption{Constraints on cluster mass for a sample of 1000 identical clusters ($M_{200} = 2\times10^{14}M_{\odot}$, at redshift 0.7) as a function of white noise levels. The beam FWHM is $1$ arcmin. Dotted line (HDV QE) show the QE forecast with a cut at $2000$ in the gradient leg~\cite{Hu:2007bt} to remove the QE bias (see Fig.~\ref{fig:Bias_sup} \JC{FIX fig ref}), dashed lines the QE case without any cuts, and solid the maximum a posterior lensing map reconstruction method of this work (MAP). The improvement in constraining power comes from the partial removal from the CMB maps of the lensing signal unrelated to the cluster, and is more prominent in polarization, as expected. The error bars indicate results obtained from 1000 simulations, showing good consistency.  \LL{error bars are obtained from the bootstrap method, should comment more on that} \bb{updated: add reference} In the QE case, we have applied a simple multiplicative correction to remove the bias in the estimate\JC{Correct?}.\JC{a further comment on the importance of the Ckk term?}}.
    \label{fig:forecast}
\end{figure}
...
To accurately assess the noise in each reconstruction, we performed additional runs of the estimators on the same simulation data but without the cluster present. To eliminate the variance due to noise introduced during the stacking process, we first subtract the noise from each individual simulation and then proceed to stack them. \LL{how do you estimate the noise?}\bb{added}
...
Hence we only binned radial component of $\alpha$.
We also present the results in terms of $\kappa_0$ in Table~\ref{tab:bias}.
As expected the QE is biased low for such massive clusters, whereas the HDV QE~\cite{Hu:2007bt} does almost unbiased reconstruction of the cluster signal, at the cost of a 20\% increase in the error of estimation.
Our iterative estimator in other hand, does manage to get rid of $65\%$ of the bias\JC{This sounds weird}, without wasting any information in the small scale regime\JC{how much was wasted with cut?} \bb{updated}.
...
We use the lensing multipoles beateen $L_{\text{min}}=100$ and $L_{\text{max}}=6000$ to obtain $\hat \kappa_0$ from Eq.~\ref{eq:kappa_0}. We compute the variance of $\hat \kappa_0$ from our set of 1000 simulations, and compare to the theoretical expectations of the variance from Eq.~\ref{eq:kappa_0_noise}.
...
\caption{Summary of results on 1000 simulations with input $\kappa_0 = 0.1285$. The table displays the mean $\kappa_0$ evaluated from 1000 simulations along with the corresponding errors. For the T and TQU channels, HDV QE results are provided, while the QE result is presented for the QU channel. Additionally, MAP estimated values are shown in the second row.
 \JC{Description missing. Fix notation of theoretical error in 3.11}\bb{updated}\JC{The two are still inconsistent to each other...}}
    \begin{tabularx}{0.15\textwidth}{|X|X|}
    \hline
    \multicolumn{2}{|c|}{HDV QE : T} \\ \hline
...
With the noise level of the CMB S4, the MAP estimator detects the cluster signal with a significance of $21.8\sigma$, representing an approximate $12\%$, $13\%$ and $20\%$ \JC{?? how was this calculated ?}enhancement compared to QE with TQU, QU and T channels respectively.
...
The MAP estimator improves the constraints on the cluster mass by a factor \LL{xxxx} compared to the HDV QE \cite{Hu:2007bt}, \JC{This is due to the absence of scale cuts in temperature, and iterative reduction of the CMB fluctuations sourced by cluster and LSS lensing (in polarization predominantly)}.

In temperature, the MAP estimator suffers from a much smaller bias than the QE, arising from the misestimate of the background gradient CMB modes. This may be interpreted by the fact that the MAP iteratively estimates the delensed CMB modes, getting more accurately the unlensed large scale gradient\JC{not certain this is necessarily a convincing explanation to me, but maybe it is}.
This permits usage of all CMB scales in the reconstruction, without the necessity of scale cuts as in \cite{Hu:2007bt} \JC{(provided, of course, that foregrounds or other contaminating signals are under control on the relevant scales).}

\JC{The MAP reconstruction used here is the same that is being developed for generic CMB lensing reconstruction.  It makes no assumption on what causes the deflections, and a reconstruction on some area of the sky will reconstruct at once all cluster signals in the area.}
The curved sky algorithm received recently considerable increase in speed and accuracy~\cite{Reinecke:2023gtp}, making full-sky analyses on perfectly doable in very reasonable time, and converging just as well on realistic, N-body simulations inputs. The MAP estimator cost is for instance much lower than the maximum likelihood estimator which requires of order $10^5$ simulations per redshift and mass \cite{Raghunathan:2017cle}. \JC{Not sure I like this last sentence, seems like unnecessary finger-pointing}

\JC{In this first work, we used for simplicity flat-sky simulations, each time with one cluster in a small patch.} Our simulations went a step further than most current analyses\JC{most ?} by including the lensing from the background large scale structures together with the cluster. This increases the variance of the QE, and somewhat less for the MAP. \LL{but is negligible for the MAP??}\JC{Probably not, since in all cases the Ckk term is still there. But simple to quantify from N0 from unlensed spectra eg} We assumed there the background LSS lensing independent from the cluster. In reality, clusters are located at the nodes of the cosmic web, and are highly correlated with overdensity regions. Dark matter simulations such as the Websky \cite{Stein:2020its} and DEMNUni \cite{Carbone:2016nzj} can be used to quantify this. \bb{ref not working}\JC{strange, does work for me.}

The normalization of the MAP lensing mass map (analogous to a Wiener-filter) requires careful handling. Indeed it cannot be obtained accurately analytically and we relied on a set of fiducial simulations to obtain it. We showed however in \cite{Legrand:2021qdu} that this normalization is independent of the true cosmology, input lensing and data noise statistics of the CMB, hence we expect this procedure to be sufficiently robust in practice.

 \LL{Do we expect more problems with the normalisation for cluster lensing, like MF or else?}\JC{cant see a showstopper there at the moment}
