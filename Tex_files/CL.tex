...
We generate a set of simulations of the CMB on flat sky patches of size \LL{xxx} deg$^2$. We lens these CMB patches with a realization of the large scale structure deflection field, together with a dark matter halo centered on the patch, following the cluster convergence profile described above. 
Our simulations have been obtained with the code LensIt\footnote{\url{https://github.com/carronj/LensIt}}.
...
\LL{This paragraph below can be put in appendix}
...
In pixel space, the unnormalized quadratic estimator of the lensing deflection field can be expressed as the product of two filtered CMB maps\JC{is minus sign right here ?} \LL{it is in the Eq.3 of Planck 2018 lensing paper}
...
\begin{equation}
    ...
    \end{split}
\end{equation}
The normalisation of the QE is chosen to obtain an unbiased estimator, and is expressed in Fourier space as $R^{\rm QE}(L)$, where we assume an isotropic response \cite{Hu:2001kj}.
This gives the normalised QE convergence field in Fourier space
\begin{equation}
    \hat \kappa^{\rm QE}(\vL) = \frac{2}{\vL^2} \frac{1}{R^{\rm QE}(L)} \, \hat \valpha^{\rm QE} \, .
\end{equation}
...
\end{equation}
In practice, the MAP lensing deflection field $\valpha^{\rm MAP}$ is found by Newton-Raphson iterations on the posterior, until convergence. Each iteration step can be obtained by running a QE with modified weights on partially delensed CMB maps  \cite[see][for more details]{Carron:2017mqf}. In our analysis, as there is no masking and the noise is isotropic, we neglect the mean field term in the lensing estimators. In practice there is a mean field that is created by the delensing procedure (the delensed noise is not isotropic anymore), but we found that this mean field contribution can be safely neglected in the absence of mask and other non-idealities. 

The normalization of this estimator is however not tractable analytically. We follow \cite{Legrand:2021qdu,Legrand:2023jne} and obtain an empirical normalization of our MAP estimator from a set of simulations.
We generate \LL{1000?} CMB flat sky patches, and lens them with random realizations of the large scale structures deflection field. Note that in these simulations we do not include any cluster. The empirical normalization is assumed to be isotropic, and is given by the cross correlation between the reconstructed $\hat \phi^{\rm MAP}$ and the input lensing potential $\phi^{\rm in}$, averaged over our set of simulations
\begin{equation}
    \mathcal{W}^{\rm MAP}_L = \left< \frac{C_L^{\phi^{\rm in}, \hat \phi^{\rm MAP}}}{C_L^{\phi^{\rm in}, \phi^{\rm in}}} \right> \; . 
\end{equation}
As shown in \cite{Legrand:2021qdu,Legrand:2023jne}, this empirical normalization is not sensitive to the cosmology or to the noise level, allowing for a robust normalization even when the true cosmology and noise statistics are poorly known.  

\JC{Plus some comment here on deflection-inversion and finufft changes} \LL{Julien I will let you do that}
...
Once we have reconstructed the convergence maps $\hat{\kappa} (\vL)$ from the observed CMB fields, we perform a matched-filtering to obtain the amplitude of the cluster convergence $\kappa_0$, for the theoretical template $\kappa_t (\vL)$ and a given reconstruction variance $N(\vL)$, following \cite{Zubeldia:2019brr}.
In other words, we define an estimator of $\kappa_0$ that minimizes the variance when comparing the theoretical template to the lensing estimation $\hat{\kappa}$ of the observed map
% To quantify the fit, we consider the covariance between different modes: $\langle{\hat{\kappa} (\vL) \hat{\kappa} (\vL')}\rangle - \langle{\hat{\kappa} (\vL)}\rangle \langle \hat{\kappa} (\vL') \rangle = \delta(\vL - \vL') N (\vL)$. This allows us to construct a minimum variance estimator for $\kappa_0$ as a function of the lensing estimation $\hat{\kappa}$ of the observed map.
...
As demonstrated in the previous section, the problem exhibits symmetry, resulting in all quantities in the aforementioned expression being functions of $L = |\vL|$. Consequently, we can derive the theoretical variance $\sigma_{\mathrm{th}}^2$ for this estimator
\begin{equation}\label{eq:kappa_0_noise}
\frac{1}{\sigma^2_{\mathrm{th}}} = \frac{1}{2\pi}\int \diff L L \frac{|\kappa_t(L)|^2}{N(L)}
\end{equation}

In these equations, the variance of the $\hat \kappa(L)$ estimate is given by $N(L) = C_L^{\kappa\kappa} + N_0(L) + N_1 (L)$. The $C_L^{\kappa\kappa}$ is the power spectrum of the background convergence of the large-scale structure, which should be treated as Gaussian noise in the context of the lensing signal from clusters\JC{plus one comment on clusters being part of LSS}. For the QE, $N_0 (L)$ is the disconnected part of the four point function of the QE power spectrum, and  $N_1 (L)$ is the connected four point function that is at first order in $C_L^{\kappa\kappa}$ \cite{Kesden:2003cc,Lewis:2006fu}.
For the MAP, we estimate the $N_0 (L)$ and $N_1 (L)$ biases iteratively, with the procedure described in \cite{Legrand:2021qdu}. From the QE $N_0 (L)$ and $N_1 (L)$ , we compute the theoretical delensing efficiency, which gives us the delensed CMB spectra. We then estimate the $N_0 (L)$ and $N_1 (L)$ biases with these delensed spectra in the QE weights and response functions, and repeat the procedure until convergence.
...
Our analysis focuses on a CMB S4-like experiment \cite{CMB-S4:2016ple}, where the forecasted noise level is  $1 \mu K$-arcmin in temperature and $\sqrt{2} \mu K$-arcmin in polarization. We assume a beam with a full width at half-maximum (FWHM) of 1 arcmin. Our flat sky patches have a pixel size of $0.3$ arcmin with 1024 pixels on a side. 
We simulate the lensing by both the large scale structures and the dark matter halo, assuming they are uncorrelated. The dark matter halo is centered on our flat sky patches. For our simulations, we use  a halo mass $M_{200} = 2\times10^{14} M_{\odot}$ and $z$ is $0.7$.

We demonstrate the application of both the quadratic estimator and the MAP estimator, as introduced in Section~\ref{sec:estimators}, to obtain the estimated $\hat{\kappa}(L)$. 
