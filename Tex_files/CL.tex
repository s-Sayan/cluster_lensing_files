...
and use the symmetric Fourier transform convention\JC{x, really?}
\begin{equation}
...
Once we have reconstructed the lensing map from observed CMB data, our objective is to fit our theoretical template $\kappa_0\kappa^t (\mathbf{L})$ to these observations $\hat{\kappa} (\mathbf{L})$ with Gaussian noise $N(\mathbf{L})$. In other words, we aim to find the best-fitting $\kappa_0$ that minimizes the variance when compared to the lensing estimation $\hat{\kappa}$ of the observed map. To quantify the fit, we consider the covariance between different modes: $\langle{\hat{\kappa} (\mathbf{L}) \hat{\kappa} (\mathbf{L}')}\rangle - \langle{\hat{\kappa} (\mathbf{L})}\rangle \langle \hat{\kappa} (\mathbf{L}') \rangle = \delta(\mathbf{L} - \mathbf{L}') N (\mathbf{L})$. This allows us to construct a minimum variance estimator for $\kappa_0$ as a function of the lensing estimation $\hat{\kappa}$ of the observed map.
...
\end{equation}
As demonstrated in the previous section, the problem exhibits symmetry, resulting in all quantities in the aforementioned expression being functions of $L = |\mathbf{L}|$. Consequently, we can derive the theoretical error $\sigma^2$ for the estimator as follows:
\begin{equation}\label{eq:kappa_0_noise}
\frac{1}{\sigma^2_{\text{th}}} = \frac{1}{2\pi}\int dL L \frac{|\kappa_t(L)|^2}{N(L)}
\end{equation}
In these equations, the noise of the $\hat \kappa(L)$ estimate is given by $N(L) = C_L^{\kappa\kappa} + N_0(L) + N_1 (L)$. The $C_L^{\kappa\kappa}$ is the power spectrum of the background convergence profile of the large-scale structure (LSS), which should be treated as Gaussian noise in the context of the lensing signal from clusters\JC{plus one comment on clusters being part of LSS}. The $N_0 (L), N_1(L)$ are known $N_0$ and $N_1$ biases in the literature \cite{Kesden:2003cc,Lewis:2006fu}.\JC{One more sentence to describe the difference between the two}
