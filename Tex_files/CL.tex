...
One difference is that the analytical predictions are calculated using curved-sky geometry, while the simulations use small boxes in the flat-sky approximation. Fig.~\ref{fig:biaspred} shows the prediction with this tool for the halo mass bias as a function of halo mass, together with our findings on this simulation set. In Fig.~\ref{fig:bias_sup_QU}, we plot the quantity in Eq.~\ref{Eq:Int_kappa0}, which is used to estimate $\kappa_0$, QU channel estimators. Intuitively, the bias in T channel QE, comes due to the apperent decrease of the background gradient due to the multiple lens effect close to the cluster centre. However, for QU channel estimator, we include all the combinations of E and B modes. If the QE was just constructed from pure EE estiomator, we would have seen similar bias like TT channel. But QU channel is actually heavily weighted from EB combination and the B map does not suffer such strong-lens magnification. Hence we see in Fig.~\ref{fig:bias_sup_QU} that the QU channel QE does not suffer from such bias. \JC{Some explanation for this here}\JC{Fig 4 before fig3 ?}\bb{done}
