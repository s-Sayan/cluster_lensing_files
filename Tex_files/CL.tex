...
The expression for the cluster convergence can then be obtained, which resembles \LL{what do you mean by ressembles?} \bb{The equation is almost the same except ours is more general including $x_{\text{max}}$ instead of $c_{200}$} Equation 27 in reference~\cite{Takada:2002qq}.
\begin{equation}
...
We rewrite the cluster convergence as a product of a normalization $\kappa_0$ and a template function $\kappa_t$, which depends on the chosen density profile of the dark-matter halo \LL{and the template depends also on the mass and redshift?} \bb{yes, the concentration parameter $c_{200}$ depends on mass and redshift, hence the template also has the dependence}, so that $\kappa_{\text{cl}} (x) = \kappa_0 \, \kappa_t (x)$. 
...
We generate \LL{1000?} \bb{yes} CMB flat sky patches, and lens them with random realizations of the large scale structures deflection field. Note that in these simulations we do not include any cluster signal. The empirical normalization is assumed to be isotropic, and is obtained from the cross correlation between the reconstructed $\hat \phi^{\rm MAP}$ and the input lensing potential $\phi^{\rm in}$, averaged over our set of simulations
\begin{equation}
...
\bb{Yes you are right! $\theta_s$ estimate comes from empirical form of $c_{200}$ which has been mensioned and referred in Eq.~\ref{eq:c200}.}
...
%Fig.~\ref{fig:int} shows the contribution of different lensing multipoles to the cluster mass signal for this configuration.
...
As discussed in the literature \cite{Maturi:2004zj, Hu:2007bt} the temperature quadratic estimator is biased low due to strong to moderate lensing close to the cluster center. 
As the strong lensing magnifies the background CMB, it seems \JC{seems or does?}to decrease the gradient of the background CMB to the weak-lensing QE. 
...
A solution to this bias, as demonstrated in \cite{Hu:2007bt}, is to use a low-pass filter for the gradient leg and only include multipoles below $\ell=2000$. 
...
In this subsection we present how our MAP estimator perform on simulations and compare it with the QE's. 
...
We run all the estimators on $1000$ simulations, stack the reconstructed maps and compare it with the input cluster signal.
...
But it only has the non-zero radial component, since cluster is circularly symmetric. 
...
We now consider a cluster of mass of $M_{200} = 2\times 10^{14}$, at $z=0.7$, which reflects roughly the expected mean mass and redshift of the CMB-S4 clusters detected with thermal SZ effect \cite{CMB-S4:2016ple} . As previously, we consider a CMB-S4 noise configuration.
We include multipoles $\ell_{\text{min}}^{\text{CMB}} = 100$ to $\ell_{\text{max}}^{\text{CMB}} = 4000$ in the CMB temperature and polarization maps for the lensing reconstruction, with the three estimators: the standard QE, the modified QE, noted HDV \cite{Hu:2007bt} with a scale cut on the temperature map at $\ell_{\rm cut}=2000$ (? to check), and the MAP estimator. The HDV estimator is only used on the temperature reconstructions.
In Figure~\ref{fig:int}, we plot the contribution of each lensing multipole to the theoretical signal to noise in Eq.~\ref{eq:kappa_0_noise}. These curves are obtained with the theoretical $N_L^0$ and $N_L^1$ of each estimator. 

We see that for both QE and MAP, the lensing scales that dominates the signal to noise are around $L\sim 2000$ in the temperature and around  $L\sim 1100$ in the polarization. The HDV estimator mainly loose information on the large scales, for $L\sim 2000$, while smaller scales are similar to the QE.
This shows that for our CMB-S4 configuration, temperature and polarization channels are bringing almost the same amount of information, albeit from different scales.  
Finally we clearly see that the MAP estimator outperforms the QE at all scales, and that this improvement is mainly coming from the polarization channels. 
...
% %For such configuration, it is important to understand which scales are important to reconstruct $\hat{\kappa}_0$ from the reconstructed $\hat{\kappa}(\ell)$. Hence 
...
%We use $L_{\text{min}}=100$ and $L_{\text{max}}=6000$ to include information from all relevant scales for our $\hat{\kappa}_0$ calculation in Eq.~\ref{eq:kappa_0}. We use such $\ell_{\text{min}}^{\text{CMB}}$ and $L_{\text{min}}$ in our calculation, so that we include modes well inside the sky-patch.
