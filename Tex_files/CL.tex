...
We run all the estimators (QE, HDV and MAP) with the temperature only channel, on $1000$ simulations, and stack the reconstructed maps. To accurately assess the noise in each reconstruction, we performed additional runs of the estimators on the same simulation data but without the cluster present.To eliminate the variance due to noise introduced during the stacking process, we first subtract the noise from each individual simulation and then proceed to stack them. \LL{how do you estimate the noise?}\bb{added}
...
As expected the QE is biased low for such massive clusters, whereas the modified QE~\cite{Hu:2007bt} with the cut in the gradient leg does almost unbiased reconstruction of the cluster signal. But it costs 20\% increase in the error of estimation.  
Our iterative estimator in other hand, does manage to get rid of $65\%$ of the bias\JC{Must quantify this}, without wasting any information in the small scale regime\JC{how much was wasted with cut?} \bb{updated}.
Therefore, in the next section, we discuss how the iterative estimator also suppresses the noise of the mass estimation compared to QE and HDV QE.
