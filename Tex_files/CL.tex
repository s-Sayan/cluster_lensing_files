...
One difference is that the analytical predictions are calculated using curved-sky geometry, while the simulations use small boxes in the flat-sky approximation. Fig.~\ref{fig:biaspred} shows the prediction with this tool for the halo mass bias as a function of halo mass, together with our findings on this simulation set. In Fig.~\ref{fig:bias_sup_QU}, we plot the quantity in Eq.~\ref{Eq:Int_kappa0}, which is used to estimate $\kappa_0$, QU channel estimators. We see that the QE does not suffer from such bias for the QU channel.
