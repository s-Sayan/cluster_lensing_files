...
\LL{added these comments:} We see that the predicted mass bias for the HDV QE is quite accurate compared to the simulations, while there seems to be a small offset for the QE. It is interesting to note that the MAP bias is very close to the HDV QE bias, while having lower variance.


\LL{Rewrote the paragraph below}
...
% Intuitively, the bias in T channel QE, comes due to the apperent decrease of the background gradient due to the multiple lens effect close to the cluster centre. However, for QU channel estimator, we include all the combinations of E and B modes. If the QE was just constructed from pure EE estiomator, we would have seen similar bias like TT channel. But QU channel is actually heavily weighted from EB combination and the B map does not suffer such strong-lens magnification. Hence we see in Fig.~\ref{fig:bias_sup_QU} that the QU channel QE does not suffer from such bias. \JC{Some explanation for this here}\JC{Fig 4 before fig3 ?}\bb{done}
