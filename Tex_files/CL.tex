...
Once we have reconstructed the lensing map $\hat{\kappa} (\mathbf{L})$ from the simulated CMB data, we fit our theoretical template $\kappa_0\kappa^t (\mathbf{L})$. We do this assuming an isotropic Gaussian noise spectrum $N(\mathbf{L})$, such that $\langle{\hat{\kappa} (\mathbf{L}) \hat{\kappa} (\mathbf{L}')}\rangle - \langle{\hat{\kappa} (\mathbf{L})}\rangle \langle \hat{\kappa} (\mathbf{L}') \rangle = \delta(\mathbf{L} - \mathbf{L}') N (\mathbf{L})$. This allows us to construct a minimum variance estimator for $\kappa_0$, given by
...
\end{equation}
Owing to spherical symmetry, all quantities but $\hat{\kappa} (\mathbf{L})$ in the aforementioned expression are functions of $L = |\mathbf{L}|$. The theoretical error $\sigma^{2,\rm th}_{\kappa_0}$ for the estimator is as follows:
...
\end{equation}
In these equations, the noise of the $\hat \kappa(L)$ estimate is given by \begin{equation}
 	N(L) = C_L^{\kappa\kappa} + N_L^{(0)} + N_L^{(1)},
 \end{equation}
where $C_L^{\kappa\kappa}$ is the power spectrum of the background convergence profile of the large-scale structure (LSS), which contributes to the Gaussian noise on the lensing signal from clusters. $N^{(0)}_L$ is the leading CMB-lensing reconstruction noise (the disconnected four-point function), and $N^{(1)}(L)$ the connected part originating from the secondary trispectrum contractions, proportional to $C_L^{\kappa\kappa}$~\cite{Kesden:2003cc,Lewis:2006fu}. On hypothetical maps where the cluster lensing signal were the only lensing signal, only $N^{(0)}_L$ would be present.\JC{How much of a difference would it make, could we just quote quickly a number?}
