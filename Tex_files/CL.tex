...
\label{sec:method}
We describe in \ref{sec:MAP} the MAP reconstruction methodology. We follow  Ref.~\cite{Carron:2017mqf} very closely, but had to adapt some details to make the algorithm work reliably at high resolution and high multipoles in the presence of clusters.
In \ref{sec:clusrec} we describe the simple matched filter we use to recover the mass of the cluster from the MAP lensing maps.

\subsection{Lensing map reconstruction}
\label{sec:MAP}
...
and use the symmetric Fourier transform convention
\begin{equation}
    T(\vell) = \int \frac{d^2 \vn}{2\pi} T(\vn) e^{-i\vell\cdot \vn}.
\end{equation}
%which we write in the compact notation $x(\vell) = \mathcal{Y} x(\vn)$.
As standard in CMB-lensing literature, we use $\vell$ for the CMB multipoles and $\vL$ for the lensing multipoles.   
...
For a given deflection field the covariance of the observed CMB fields is
\begin{equation}
...
\begin{equation}
    \hat \kappa^{\rm QE}(\vL) = \frac{2}{\vL^2} N(\vL) \hat \valpha_{\vL}^{\rm QE} \, .
\end{equation}
...
\end{equation}
In practice, the MAP lensing deflection field $\valpha^{\rm MAP}$ is found by Newton-Raphson iterations on the posterior, until convergence, using the gradient  with respect to $\valpha$ of the likelihood as search direction.

The main term of this gradient is a QE with modified weights on partially delensed CMB maps~\cite[see][for more details]{Carron:2017mqf}. This is the term originating from the quadratic piece of the right-hand side in Eq.~\eqref{eq:loglik}, whose task is to capture residual lensing signal. The gradient of the second term in Eq.~\eqref{eq:loglik}, data-independent, is the `mean-field', which removes signature of lensing-unrelated anisotropies from the quadratic piece. 
In a idealized situation like the one in this paper, the only source of mean-field is the anisotropy of the delensed-noise: by construction, at each iteration, the CMB is delensed in order to reduce the observed CMB anisotropy. Doing this the noise map is also remapped, creating some level of anisotropies in the maps even if there were none before. Hence, the 

Unfortunately, the normalisation of the MAP estimator is not tractable analytically. We follow \cite{Legrand:2021qdu,Legrand:2023jne} and obtain an empirical normalisation of our MAP estimator from a set of simulations\JC{a tiny bit more details on how you do that might be in order here,  a sentence, or one equation}. As shown in \cite{Legrand:2021qdu,Legrand:2023jne}, this empirical normalisation is not sensitive to the true cosmological model, input lensing map or data noise statistics, allowing for a correct normalisation even when the fiducial ingredients of the reconstruction are a poor match to the true ones.
\JC{Plus some comment here on deflection-inversion and finufft changes}
\JC{Plus neglect of delensed-noise mean-field}
...
\hat{\kappa}_0 = \frac{\int \frac{d^2\mathbf{L}}{(2\pi)^2} \frac{\kappa^t(\mathbf{L}) \hat \kappa(\mathbf{L})}{N(\mathbf{L})}}{\int\frac{d^2\mathbf{L}}{(2\pi)^2} \frac{|\kappa^t(\mathbf{L})|^2}{N(\mathbf{L})}}.
\end{equation}
...
%To estimate the mass of the cluster, we employ matched filtering, as described by Eq. \ref{eq:ft_anal1} and \ref{eq:ft_anal2}. 
We test both the quadratic estimator and the iterative or MAP estimator, as explained in the previous subsection, to obtain the estimated $\hat{\kappa}(\vL)$.
Our analysis focuses on a CMB S4-like experiment, with a white noise level of 1\JC{right?} $\mu \rm{K}$-arcmin. We assume a Gaussian beam with a full width at half-maximum (FWHM) of 1 arcmin.
In this first study we employ the flat-sky approximation, using square sky patches with $1024$ pixels of size $0.3$ arcmin on a side. 
We lens these CMB patches with a realization of the large scale structure deflection field, together with a dark matter halo  in the centre of the patch, following the cluster convergence profile described earlier. 
