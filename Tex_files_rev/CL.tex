...
\end{equation}
\bb{Due to the proportionality between $\kappa_0$ and $M_{200}$ in Eq.~\ref{eq:tracer}, we constrain the quanity $\frac{\hat{\kappa}_0}{\sigma_{\kappa_0}}$ in order to get the mass contraint, $\frac{\hat{M}_{200}}{\sigma_{M_{200}}}$. \bb{This quantity is reffered to as the signal to noise ratio (SNR) troughout this paper.} For estimating inverse variance,} we often use the curved-sky expression
...
To assess the noise in each reconstruction, we performed additional runs of the estimators on the same simulation, with the same CMB and LSS realization, but without the cluster present. To reduce the variance during the stacking process, for each reconstructed map we first subtract the convergence estimated from the simulation without the cluster, before stacking them. \bb{However, the analysis with real data does not allow for this approach. We demonstrate it here to visually highlight the bias more distinctly, as the LSS lensing signal and noise would otherwise obscure such bias visually. This is purely for illustrative purposes. Notably, we do not use this trick when estimating the mass in Fig 3.} 
...
In the lower panel of Figure~\ref{fig:Bias_sup}, we also plot as the dotted and dashed analytic prediction for \bb{the QE and HDV QE reconstruction of the cluster signal in L space}.  We do this essentially by brute-force calculation of the QE and HDV QE expectation values, varying CMB and LSS lensing fluctuations, while keeping the cluster lensing deflection fixed, as described in some more details in appendix~\ref{A1}.
...
We now consider 1000 simulations with a cluster mass of $M_{200} = 2 \times 10^{14} M_{\odot}$ and $z=0.7$, closer to the expected mean values of the CMB-S4 clusters. \bb{We reconstruct the lensing signal with scales between  $\ell_{\text{min}}^{\, \text{CMB}}=100$ and $\ell_{\text{max}}^{\, \text{CMB}} = 4000$, for both polarization and temperature maps.}
