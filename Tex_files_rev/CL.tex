...
Note that \bb{CMB lensing reconstruction is not free from systematics}, mainly due to astrophysical contaminations, such as the own SZ emission of the cluster \cite{Madhavacheril:2018bxi, DES:2018myw, Patil_2020}.
\st{However, at the small scales relevant for CMB lensing, polarized foreground emission is expected to be negligible.}
\LL{However, the reconstructed CMB lensing signal mainly comes from the small scales of the CMB, where the polarized foreground emission are expected to be negligible.} \LL{find a ref for that?} 
...
\bb{We use a truncated version~\cite{Zubeldia:2019brr} of the Navarro-Frenk-White profile \cite[NFW,][]{Navarro:1995iw} to model the cluster mass distribution in our analysis.} In this model, the density of the galaxy cluster is
...
\bb{One can write the following relation between $\rho_0$ and $\rho_{\text{crit}}$~\cite{Oguri_2011}},
...
\end{equation}
where $c_{200} = R_{200}/r_s$ is the concentration parameter. \bb{In such spherically symmetric model, the concentration parameter} has the following empirical dependence on the cluster mass and redshift \cite{Duffy:2008pz, Geach:2017crt}
...
The gravitational field created by the mass distribution in the \bb{\st{late-time}} Universe causes deflection of CMB photons. This phenomenon is referred to as weak lensing of the CMB~\cite{Lewis:2006fu}. As a result of weak lensing, the original CMB signal is remapped on the sky. We denote the quantities for the unlensed \LL{(or primordial)} CMB as $X (\hat{\mathbf{n}})$  and for the lensed CMB as $\widetilde{X} (\hat{\mathbf{n}})$, respectively. Here, $X$ can represent temperature ($T$) or polarization Stokes parameters ($Q$ and $U$). Assuming the flat-sky approximation, the remapping of the unlensed quantity can be expressed as
\begin{equation}
\widetilde{X} (\hat{\mathbf{n}}) = X (\hat{\mathbf{n}} + \boldsymbol{\alpha}(\hat{\mathbf{n}})),
\end{equation}
where $\boldsymbol{\alpha} (\hat{\mathbf{n}})$ is the deflection field, which can be expressed as the gradient of the lensing potential: $\boldsymbol{\alpha} (\hat{\mathbf{n}}) = \boldsymbol{\nabla}_{\hat{\mathbf{n}}}\phi(\hat{\mathbf{n}})$, neglecting the LSS lensing rotational component (second order in the scalar perturbations). The convergence $\kappa (\hat{\mathbf{n}})$ is the most relevant quantity we work with~\cite{Lewis:2006fu} \LL{Sayan did you provide the references for Eqs. (2.5), (2.6), (2.7), (2.8), and (2.9) ? Lensing is all in Lewis and Challinor 2006 I believe, and cluster profiles you can find in the litterature that we already cited}
\begin{equation}
...
\end{equation}
\bb{\st{Hence, $\kappa_0$ works as tracer of the cluster mass and constraining the signal to noise ratio (SNR) for $M_{200}$ boils down to constraining the same quantity for $\kappa_0$, $\displaystyle \frac{\sigma_{\kappa_0}}{\kappa_0} = \frac{\sigma_{M_{200}}}{M_{200}}$.}} 
...
We generate 1000 CMB flat sky patches, and lens them with \bb{Gaussian} random realizations of the large scale structures deflection field. Note that in these simulations we do not include any cluster signal. The empirical normalization is assumed to be isotropic, and is obtained from the cross correlation between the reconstructed $\hat \kappa^{\rm MAP}$ and the input convergence $\kappa^{\rm in}$, averaged over our set of simulations
\begin{equation}
...
\end{equation}
\bb{This is the matched filter that has been mentioned throughout our paper. It is essential to note that the characteristics of the matched filter are inherently linked to the assumed cluster profile, as represented by the parameter $\kappa_t$.} Owing to spherical symmetry, all quantities but $\hat{\kappa} (\vL)$ in the aforementioned expression are functions of $L = |\vL|$. The theoretical error $\sigma^{2,\rm th}_{\kappa_0}$ for the estimator is as follows~\cite{Zubeldia:2019brr}:
...
\end{equation}
\bb{Due to the proportionality between $\kappa_0$ and $M_{200}$ in Eq.~\ref{eq:tracer}, we constrain the quanity $\frac{\hat{\kappa}_0}{\sigma_{\kappa_0}}$ in order to get the mass contraint, $\frac{\hat{M}_{200}}{\sigma_{M_{200}}}$. \LL{Add the explicit equation of the SNR here.} For estimating inverse variance,} we often use the curved-sky expression
...
\bb{In Fig.~\ref{fig:int}, we show the contribution of each lensing multipole to the theoretical signal to noise of Eq.~\ref{eq:kappa_0_noise}. The y axis is the integrand at each L, which must be integrated to obtain the inverse noise in cluster mass estimation.} These curves are obtained with the theoretical $N_L^{(0)}$ and $N_L^{(1)}$ of each estimator, considering a  CMB-S4 like experiment with white noise level of $1 \mu \rm{K}$-arcmin in temperature and $\sqrt{2} \mu \rm{K}$-arcmin in polarization and a beam full width at half-maximum (FWHM) of 1 arcmin.
...
Our flat sky patches have a pixel size of $0.3$ arcmin with 1024 pixels on a side. \bb{\st{We simulate the lensing by both the large scale structures and the dark matter halo, assuming (slightly wrongly) they are uncorrelated. The dark matter haloes are in the center of the patches.}  The simulations recreate observed lensed sky patches through a two-stage lensing process. First the a Gaussian random realisation of the unlensed primordial CMB patch is lensed by a Gaussian random realisation of a LSS lensing potential. Subsequently, this once-lensed map is further lensed by a circularly symmetric \st{non-Gaussian} cluster convergence field at the center of the sky-patch. It's important to note that we assume the lensing effects from both the LSS and the cluster to be independent, acknowledging a slight deviation from actuality in this assumption. }
...
\bb{Since the weak lensing signature, manifested as a dipole-like pattern in the difference between the lensed and unlensed sky, is sensitive to both the strength of the gradient and the mass of the cluster, it decreases that signature as well.} \LL{it is not very clear to me what you said.}
...
\bb{\st{Since the bias is more prominent for massive clusters and small CMB scales, we choose here the clusters parameters to be $M_{200} = 4 \times 10^{14} M_{\odot}, z=0.7$ and use CMB multipoles from $\ell_{\text{min}}^{\, \text{CMB}}=100$ to $\ell_{\text{max}}^{\, \text{CMB}} = 5000$ for the reconstruction.}}
...
To assess the noise in each reconstruction, we performed additional runs of the estimators on the same simulation, with the same CMB and LSS realization, but without the cluster present. To reduce the variance during the stacking process, for each reconstructed map we first subtract the convergence estimated from the simulation without the cluster, before stacking them. \bb{It is important to note here that the analysis with real data doesn't allow for this approach. We demonstrate it here to visually highlight the bias more distinctly, as the LSS lensing signal and noise would otherwise obscure such bias visually. This is purely for illustrative purposes. Notably, we refrain from this method when estimating the mass in Fig 3.} \LL{Notably, we do not use this trick when estimating the mass.}
...
Hence we only stacked the radial component of $\alpha$. \bb{The oscillatory nature of this deflection profile can be attributed to the methodology employed i.e. the lensing reconstruction is conducted in harmonic space and this specific profile has been reconstructed utilizing the lensing multipoles up to a maximum limit of $L_{\text{max}}=5000$. However, it is important to note that to achieve a flawless reconstruction of the profile, one would need to extend the range of lensing multipoles used to an infinite limit, i.e., $L_{\text{max}}= \infty$.}
...
\end{equation}
which is the unnormalized contribution of each and every multipole to the mass estimate in Eq.~\ref{eq:kappa_0} (empirically, errors are independent to a good approximation). As expected the QE is biased low for such massive clusters, whereas the HDV QE~\cite{Hu:2007bt} does almost unbiased reconstruction of the cluster signal on all scales, at the cost of \bb{\st{a 20\% increase in the standard deviation of the mass estimation} wasting information at small angular scale.}
...
We now consider 1000 simulations with a cluster mass of $M_{200} = 2 \times 10^{14} M_{\odot}$ and $z=0.7$, closer to the expected mean values of the CMB-S4 clusters. \bb{\st{We reconstruct the lensing signal with scales between  $\ell_{\text{min}}^{\, \text{CMB}}=100$ and $\ell_{\text{max}}^{\, \text{CMB}} = 4000$, for both polarization and temperature maps.}} \LL{I would keep that here (it is ok to repeat that for clarity)}
...
\caption{Summary of results on 1000 simulations with input $\kappa_0 = 0.1285$, corresponding to a cluster mass of $M_{200} = 2 \times 10^{14} M_{\odot}$ at $z=0.7$. The table displays the mean $\hat \kappa_0$ evaluated from 1000 simulations along with the corresponding errors $\sigma_{\kappa_0}$. \bb{The errors in  $\sigma_{\kappa_0}$ have been obtained using bootstraping method.} For the T and TQU estimators, HDV QE results are provided, while the QE result is presented for the QU estimator. Additionally, MAP estimated values are shown in the second row. $\sigma_{\kappa_0}^{\text{th}}$ depicts the theoretical error prediction given by Eq.~\ref{eq:kappa_0_noise}. See also Fig.~\ref{fig:forecast}. \bb{SS: Table has been modified}}
    ...
    $\sigma_{\kappa_0}$ & 0.0088 $\pm$0.0002  \\\hline
    ...
    $\sigma_{\kappa_0}$ & 0.0068 $\pm$0.0002  \\\hline
