...
class profile(object):
    ...
    def kappa_map(self, M200, z, shape, lsides, xmax=None):
        ...
        if shape[0] % 2 == 0 and shape[1] % 2 == 0:
            X, Y =  np.meshgrid(np.concatenate((np.arange(0,shape[0]/2), np.arange(-shape[0]/2 +1,1))), np.concatenate((np.arange(0,shape[1]/2), np.arange(-shape[1]/2 + 1,1))))
        else:
